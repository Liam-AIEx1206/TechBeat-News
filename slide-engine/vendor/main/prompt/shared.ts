...
export function formatDesignContract(contract?: DesignContract): string {
  ...
  const lines = [
    ...
    `- Palette (MANDATORY — use these EXACT hex tokens; do NOT invent, lighten, darken, or substitute near-equivalents, and do NOT re-theme colors to match the topic): ${contract.palette.join(', ')}`,
    `- Every page MUST visibly use the palette's accent colors (not only its background/neutral tones). Distribute the accents across titles, dividers, highlights, icons, and chart series so the deck is unmistakably THIS palette — never a generic single-hue wash. Style check before writing: if a color is not one of the palette tokens (or a transparent overlay of one), replace it with the nearest palette token.`,
